function [E, C] = impAM(a1, path)
%impAM   Continuous and impulse components of a matrix.     , A3
...
[E(1,:), C(1,:), A2] = AM( a1 );  % Continuous component
...
disp(imp)
      ...
      for row = 1 : max_row - imp+2
         ...
         for col = row% TODO: 1:N - imp+2
            ...
            [e, c] = AM( A2(choice_row, choice_col) );
            e = e*mean(c);
...
%    [E(imp,UC), C(imp,UR), A2(UR,UC,imp)] = AM( A1(UR,UC,imp) );
   [E(imp,UC), C(imp,UR), A2(UR,UC)] = AM( A2(UR,UC) );
